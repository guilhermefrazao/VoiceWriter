...
class MainEditorMenu():
    ...
    async def open_explorer(self, e: ft.Event[ft.Button]):
        if e.control.data == True:
            path_editor = await ft.FilePicker().get_directory_path()
    ...
    def main(self, page: ft.Page):  
        ...
        options_container_1 = ft.Column(
            spacing=10,
            controls=[
                Containers().generic_text_container_with_right_button("Create new vault", "Create a new vault under a folder.", "Create", lambda e : self.fade(animation_switcher, options_container_1, options_container_2)),
                Containers().generic_text_container_with_right_button("Open a Folder", "Open Folder with files.", "Open", self.open_explorer, True)
                ]
            )     
        ...
        options_container_2 = ft.Column(
            spacing=10,
            controls=[
                ft.Column(spacing=2, horizontal_alignment=ft.CrossAxisAlignment.START, controls=[ft.IconButton(icon=ft.Icons.ARROW_BACK, icon_color="white", on_click=lambda e: self.fade(animation_switcher, options_container_1, options_container_2)), ft.Text("Back", size=16, color="grey")]),
                tf1 := Containers().generic_text_container_with_right_text_field("Vault name", "Pick a name to gain Aura.", "Aura name"),
                tf2 := Containers().generic_text_container_with_right_button("Location", "Pick a place to create the Aura + Ego", "Browse", self.open_explorer, False),
